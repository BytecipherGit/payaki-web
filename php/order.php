...
if(checkloggedin()) {
    ...
    $rows = ORM::for_table($config['db']['pre'].'transaction')
        ...
        ->order_by_desc('id')
        ->find_many();
    // Your join query with a WHERE condition
    //     $rows = ORM::for_table('ad_shop_order')
    //         ->join('ad_shop_order_item', ['u.id', '=', 'ad_shop_order_item.order_id'])
    //         ->select('ad_shop_order.order_at as order_date,ad_shop_order_item.item_price,ad_shop_order_item.quantity')
    //         ->where('ad_shop_order.member_id', $_SESSION['user']['id']) // Example WHERE condition
    //         ->find_many();
    //         echo '<pre>';
    //         print_r($rows);
    //         die;
