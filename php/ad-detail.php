...
if ($num_rows > 0) {
    ...
    foreach($q_result as $customdata){
        ...
        $field_data = escape_html($customdata['field_data']);
        ...
        if($field_type == 'checkboxes'){
            ...
            if($custom_fields_title != ""){
                ...
                $item_checkbox[$field_id]['icon'] = $custom_fields_icon;
        ...
        if($field_type == 'textarea') {
            ...
            $item_custom_textarea[$field_id]['icon'] = $custom_fields_icon;
        ...
        if($field_type == 'radio-buttons' or  $field_type == 'drop-down') {
            ...
            $item_custom[$field_id]['icon'] = $custom_fields_icon;
    ...
    if($info['tag'] != ""){
        ...
        foreach ($tag as $val)
        {
            ...
            $tag2[] = '<li><a href="'.$link['LISTING'].'?keywords='.$tagTrim.'">'.$val.'</a> </li>';
        }
...
if (count($result1) > 0) {
    ...
    foreach ($result1 as $info1 ) {
        ...
        $currency_code = get_countryCurrecny_by_code($info1['country']);
        $price = price_format($info1['price'],$currency_code);
        ...
        $item[$info1['id']]['catlink'] = $link['SEARCH_CAT'].'/'.$get_main['slug'];
        $item[$info1['id']]['subcatlink'] = $link['SEARCH_CAT'].'/'.$get_main['slug'].'/'.$get_sub['slug'];
    ...
    if($config['recaptcha_mode'] == 1) {
        if (isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
            ...
            if (!$responseData->success) {
                $errors++;
                $error = $recaptcha_error = $lang['RECAPTCHA_ERROR'];
                $recaptcha_error = "<span class='status-not-available'> " . $recaptcha_error . ".</span>";
            }
        } else {
            $errors++;
            $error = $recaptcha_error = $lang['RECAPTCHA_CLICK'];
            $recaptcha_error = "<span class='status-not-available'> " . $recaptcha_error . ".</span>";
        }
    }
    if($errors == 0) {
