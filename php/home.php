...
if($config['color_switcher'] == "1"){
    if(isset($_GET['theme'])){
        $theme = $_GET['theme'];
        if(file_exists(ROOTPATH."/templates/".$theme)){
            setcookie ("Quick_theme",$theme,0,"/");
            $config['tpl_name'] = $theme;
        }
    }
}
...
// get recent blog
$sql = "SELECT
MAX(b.id) id, MAX(b.title) title, MAX(b.author) author, MAX(u.name) name, MAX(u.username) username, MAX(u.image) author_pic, GROUP_CONCAT(c.title) categories, GROUP_CONCAT(c.slug) cat_slugs
