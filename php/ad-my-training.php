 ...
 WHERE status = 'active' AND hide = '0' AND user_id = '".$_SESSION['user']['id']."' ";
    ...
    if ($result) {
        foreach ($result as $info)
        {
            ...
            $items[$info['id']]['catlink'] = $link['SEARCH_CAT'].'/'.$get_main['slug'];
            $items[$info['id']]['subcatlink'] = $link['SEARCH_CAT'].'/'.$get_main['slug'].'/'.$get_sub['slug'];
    ...
    $pagging = pagenav($total,$page,$limit,$link['MYTRAININGADS']);
    ...
    // Output to template
    $page = new HtmlTemplate ('templates/' . $config['tpl_name'] . '/ad-my-training.tpl');
