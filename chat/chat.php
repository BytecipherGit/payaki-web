				...
				if (!empty($senderId) && !empty($receiverId)) {
					...
					<section id="messages_container" class="col-md-4 conversations-section chat-box-left">
						...
						<div class="search-user">
							...
							<input class="form-control mr-sm-2 ph-searchbar" id="js-searchUser" type="search"
						...
						<ul class="user-list">
							...
							if (DB::_query('SELECT ad_user.username FROM ad_user, ad_custom_messages WHERE ad_custom_messages.receiver = ad_user.id OR ad_custom_messages.sender = ad_user.id AND ad_user.id = :user_id', ['user_id' => $receiverId])) {
								...
								$usernames = DB::_query('SELECT DISTINCT ad_user.id,ad_user.username,ad_user.image FROM ad_custom_messages, ad_user WHERE (ad_custom_messages.receiver = :userid OR ad_custom_messages.sender = :userid) AND (ad_custom_messages.receiver = ad_user.id OR ad_custom_messages.sender = ad_user.id)', ['userid' => Login::isLogged()]);
