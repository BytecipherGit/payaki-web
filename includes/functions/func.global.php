...
function load_all_option_in_template(&$config){
    ...
    $info = ORM::for_table($config['db']['pre'].'adsense')->find_many();
    foreach ($info as $data){
        $status = $data['status'];
        $slug = $data['slug'];
        $large_track_code = $data['large_track_code'];
        $tablet_track_code = $data['tablet_track_code'];
        $phone_track_code = $data['phone_track_code'];
        ...
        if($status=='1'){
            ...
            $config['AD_'.$slug.'_STATUS'] = 1;
        }else{
            $config['AD_'.$slug.'_STATUS'] = 0;
        }
        $config['AD_'.$slug] = $advertise_tpl;
...
function fileUpload($path,$files,$type_file,$title,$reqwid,$reqhei,$Anysize=false,$unlink=null){
    ...
    if($type_file == "image"){
        ...
        if($Anysize){
            ...
            $watermark_path = ROOTPATH.'/storage/logo/favicon.png';
...
function run_cron_job(){
    ...
    if((time()-$cron_exec_time) > $cron_time) {
        ...
        $ads_closed = ORM::for_table($config['db']['pre'].'product')
            ->where_any_is(
                array(
                    array('status' => 'expire', 'expire_date' => $start_time),
                    array('expire_date' => '<')
                )
            )
            ->count();
        ...
        if($config['delete_expired'] == 1){
            ...
            ORM::for_table($config['db']['pre'].'product')
                ->where_any_is(
                    array(
                        array('status' => 'expire', 'expire_date' => $expiry_time),
                        array('expire_date' => '<')
                    )
                )
                ->delete_many();
...
// END UPDATE ONLINE STATUS
        ...
        ORM::for_table($config['db']['pre'].'transaction')
            ->where_any_is(
                array(
                    array('status' => 'pending', 'transaction_time' => $expiry_time),
                    array('transaction_time' => '<')
                )
            )
            ->delete_many();
        ...
        $query = "UPDATE `".$config['db']['pre']."product` SET `featured` = '0' WHERE `featured_exp_date` < '" . $now . "'";
        $pdo->query($query);

        $query = "UPDATE `".$config['db']['pre']."product` SET `urgent` = '0' WHERE `urgent_exp_date` < '" . $now . "'";
        $pdo->query($query);

        $query = "UPDATE `".$config['db']['pre']."product` SET `highlight` = '0' WHERE `highlight_exp_date` < '" . $now . "'";
        $pdo->query($query);
