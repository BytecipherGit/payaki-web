...
function get_language_list($selected = "", $selected_text = 'selected', $active = false)
{
    ...
    foreach ($result as $info) {
        ...
        if ($selected != "") {
            if ($selected == $info['id'] or $selected == $info['code'] or $selected == $info['file_name']) {
...
function get_customField_by_id($id)
{
    ...
    $info = ORM::for_table($config['db']['pre'] . 'custom_fields')
        ->select_many('custom_title', 'icon', 'translation_lang', 'translation_name')
        ->where('custom_id', $id)
        ->find_one();
    ...
    $result = array();
    $result['title'] = $custom_fields_title;
...
function get_customOption_by_id($option_id)
{
    ...
    if ($config['lang_code'] != 'en' && $config['userlangsel'] == '1') {
        ...
        if (isset($customoption['title']) && $customoption['title'] != "") {
            ...
            if (isset($product_id)) {
                ...
                if ($exist > 0) {
                    ...
                    if ($field_data != "") {
                        ...
                        $field_insert->product_id = validate_input($product_id);
                        $field_insert->field_id = validate_input($field_id);
                        $field_insert->field_type = validate_input($field_type);
                        $field_insert->field_data = validate_input($field_data);
                        $field_insert->save();
                    }
                }
            }
        ...
        if ($info['custom_type'] == 'radio-buttons') {
            ...
            foreach ($options as $key3 => $value3) {
                ...
                if ($config['tpl_name'] == 'thenext-theme') {
                    ...
                    $radiobtn .= '<div class="radio-wrap"><label for="' . $value3 . $i . '"><input class="with-gap" type="radio" name="custom[' . $info['custom_id'] . ']" id="' . $value3 . $i . '" value="' . $value3 . '" data-name="' . $info['custom_id'] . '" ' . $checked . ' />' . $option_title . '</div>';
                ...
                if ($config['tpl_name'] == 'thenext-theme') {
                    ...
                    $Checkbox .= '
                        <div class="checkbox checkbox-inline checkbox-primary">
                            <label for="' . $value4 . $j . '" ><input type="checkbox" name="custom[' . $info['custom_id'] . '][]" id="' . $value4 . $j . '" value="' . $value4 . '" data-name="' . $info['custom_id'] . '" ' . $selected . ' /> ' . $option_title . '</label>
                        </div>';
                } else {
                    $Checkbox .= '
                        <div class="checkbox checkbox-inline checkbox-primary">
...
function get_category_id_by_slug($slug)
{
    ...
    if (!empty($info)) {
        ...
        $info = ORM::for_table($config['db']['pre'] . 'category_translation')
            ->select_many('translation_id', 'lang_code')
            ->where(array(
                'slug' => $slug,
                'category_type' => 'main',
            ))
            ->find_one();
        change_user_lang($info['lang_code']);
...
function get_maincategory($selected = "", $selected_text = 'selected', $adcount = false)
{
    ...
    foreach ($result as $info) {
        ...
        $cat_slug = $info['slug'];
        $cat[$info['cat_id']]['link'] = $link['SEARCH_CAT'] . '/' . $cat_slug;
...
function get_subcategories()
{
    ...
    foreach ($info as $key => $value) {
        ...
        if ($config['lang_code'] != 'en' && $config['userlangsel'] == '1') {
            ...
            if (isset($subcat_trans['title']) && $subcat_trans['title'] != "") {
...
function get_subcat_of_maincat($category_id, $adcount = false, $selected = "", $selected_text = 'selected')
{
    ...
    foreach ($result as $info) {
        ...
        $subcat[$info['sub_cat_id']]['link'] = $link['SEARCH_CAT'] . '/' . $category_slug . '/' . $subcat_slug;
...
function get_items($userid = null, $status = null, $premium = false, $page = null, $limit = null, $sort = "id", $location = false, $order = false, $sort_order = "DESC")
{
    ...
    if($premium){
        ...
        if($where == '')
            // $where .= "where ($grqry or p.featured = '1' or p.urgent = '1' or p.highlight = '1')";
            $where .= "where (p.featured = '1' or p.urgent = '1' or p.highlight = '1')";
        else
            // $where .= " AND ($grqry or p.featured = '1' or p.urgent = '1' or p.highlight = '1')";
    ...
    if ($result) {
        foreach ($result as $info) {
            ...
            $item[$info['id']]['desc'] = strlimiter(strip_tags($info['description']), 110);
...
function get_resubmited_items($userid = false, $status = null, $page = null, $limit = null, $sort = "id")
{
    ...
    if ($result) {
        foreach ($result as $info) {
            ...
            $item[$info['id']]['catlink'] = $link['SEARCH_CAT'] . '/' . $catslug;

            $item[$info['id']]['subcatlink'] = $link['SEARCH_CAT'] . '/' . $catslug . '/' . $subcatslug;
...
function renew_item_by_userid($userid = null)
{
    global $config;
    $pdo = ORM::get_db();
    $group_id = get_user_group($userid);
...
function payment_success_save_detail($access_token)
{
    ...
    if ($payment_type == "subscr") {
        ...
        if ($payment_status == "Completed") {
            ...
            headerRedirectScript($redirect_url);
            //message($lang['SUCCESS'],$lang['PAYMENTSUCCESS'],$link['TRANSACTION']);
            exit();
...
/**
New Update for login with mobile number
 * *
 */
...
{
    global $config;
    $sms_api_provider = get_option("sms_provider");

    if ($sms_api_provider == 'twilio') {
        $TWILIO_FROM = get_option("twilio_from");
        $TWILIO_ACCOUNT_SID = get_option("twilio_account_sid");
        $TWILIO_AUTH_TOKEN = get_option("twilio_auth_token");
        $apiUrl = 'https://api.twilio.com/2010-04-01/Accounts/' . $TWILIO_ACCOUNT_SID . '/Messages.json';
        ...
        curl_close($ch);
        $return_res = json_decode($response);
        if ($return_res->status == 'queued') {
            echo 'success';
            die();
        }
        echo $response;
    ...
    } elseif ($sms_api_provider == 'nexmo') {
        $apiUrl = 'https://rest.nexmo.com/sms/json';
        $VONAGE_FROM = get_option("vonage_from");
        $VONAGE_API_KEY = get_option("vonage_api_key");
        $VONAGE_API_SECRET = get_option("vonage_api_secret");
        $data = array(
            "text" => $message,
            "from" => $VONAGE_FROM,
            "to" => $recipient_no,
...
function insert_mobile_number($user_id, $mobile_no)
{
    global $config, $lang, $link;
    if (!empty($mobile_no)) {
        ...
        if ($checkPrev) {
            ...
            $insert_mobile->user_id = $user_id;
            $insert_mobile->mobile_number = $mobile_no;
        ...
        if(empty($config['sms_otp_template'])){
        $sms_body = "Hello,".$username." Your OTP CODE is ".$rand_no;
        }else{
        ...
        $send = sendSMS($mobile_no, $sms_body);
        if($send){
        return true;
        }else{
        return false;
