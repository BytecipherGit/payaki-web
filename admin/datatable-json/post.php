...
foreach ($queryRecords as $row) {
    ...
    $row1 = '<td class="none">'.$username.'</td>';
    $row2 = '<td class="none">'.$row['cityname'].'</td>';
    $row3 = '<td class="hidden-xs hidden-sm">'.$ad_created_at.'</td>';
    $row4 = '<td class="hidden-xs hidden-sm">'.$status.'</td>';

    $row5 = '<td class="text-center">
                <div class="btn-group">
                <a href="#" class="btn btn-xs btn-default login-as-user" data-redirect-url="'.$link['EDIT-AD'].'/'.$id.'" data-user-id="'.$user_id.'">Edit in Frontend</a>
                '.$approved_button.'
