...
if (!empty($info)) {
    ...
    $item_catlink = $link['SEARCH_CAT'].'/'.$item_category_slug;

    $item_subcatlink = $link['SEARCH_CAT'].'/'.$item_category_slug.'/'.$item_sub_category_slug;
    ...
    foreach ($tag as $val)
    {
        ...
        $val = trim($val);
        $tag2[] = '<a href="'.$link['LISTING'].'?keywords='.$val.'" class="bylabel bylabelmini" target="_blank">'.$val.'</a>';
    }
