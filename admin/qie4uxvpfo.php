 ...
 * @author Bylancer
 * @version 10.5.1
 * @Updated Date: 6/May/2023
 * @url https://codecanyon.net/item/quickad-classified-ads-php-script/19960675
 * @Copyright (c) 2015-23 Devendra Katariya (bylancer.com)
 */
...
if(isset($_GET['action'])){
    ...
    if ($_GET['action'] == "approveReview") { approveReview(); }
    if ($_GET['action'] == "deleteReview") { deleteReview(); }
    ...
    if ($_GET['action'] == "CustomField_Icon_Url") { CustomField_Icon_Url(); }
    if ($_GET['action'] == "CustomField_Icon_Save") { CustomField_Icon_Save(); }
...
function approveReview()
{
    ...
    $id = escape_html($_POST['id']);
    if (trim($id) != '') {
        if(check_allow()){
            $con->query("UPDATE `".$config['db']['pre']."reviews` set publish='1' WHERE `reviewID` = '" . $id . "'");
            echo 1;
        }else{
            echo 1;
            die();
...
function deleteReview()
{
    ...
    if(isset($_POST['id']))
    {
        $_POST['list'][] = escape_html($_POST['id']);
    }

    if (is_array($_POST['list'])) {

        $count = 0;
        $sql = "DELETE FROM `".$config['db']['pre']."reviews` ";

        foreach ($_POST['list'] as $value)
        {
            if($count == 0)
            {
                $sql.= "WHERE `reviewID` = '" . escape_html($value) . "' ";
            }
            else
            {
                $sql.= " OR `reviewID` = '" . escape_html($value) . "' ";
            }
...
/********************************************
 * Language Translation Categories
 *******************************************/
    ...
    if(check_allow()){
        ...
        if($cattype == "main"){
            $result = ORM::for_table($config['db']['pre'].'catagory_main')
                ->select_many('cat_name','slug')
                ->where('cat_id',$id)
                ->find_one();
            $title_english = $result['cat_name'];
            $slug_english = $result['slug'];
        }elseif($cattype == "sub"){
            $result = ORM::for_table($config['db']['pre'].'catagory_sub')
                ->select_many('sub_cat_name','slug')
                ->where('sub_cat_id',$id)
                ->find_one();
            $title_english = $result['sub_cat_name'];
            $slug_english = $result['slug'];
        }
        elseif($cattype == "custom_option"){
            $result = ORM::for_table($config['db']['pre'].'custom_options')
                ->select('title')
                ->where('option_id',$id)
                ->find_one();
        ...
        foreach ($_POST['value'] as $items) {
            ...
            if($title == ""){
                ...
                $title = $trans->translate($source, $target, $title_english);
            }

            if($cattype != "custom_option"){
                if($slug == "")
...
/********************************************
 * Language Translation Custom Fileds
 *******************************************/
    ...
    if(isset($_POST['id'])){
        if(check_allow()){
            ...
            $info = ORM::for_table($config['db']['pre'].'custom_fields')
                ...
                ->where('custom_id' , $id)
                ->find_one();
            $title_english = $info['custom_title'];
            ...
            foreach($_POST['trans_lang'] as $code){
                ...
                if($trans_name[$i] == ""){
                    $trans = new GoogleTranslate();
                    $title[$i] = $trans->translate($source, $target, $title_english);
                }else{
                    $title[$i] = $trans_name[$i];
                }
                $i++;
            }
            ...
            $rowcount = ORM::for_table($config['db']['pre'].'custom_fields')
                ->where('custom_id' , $id)
                ->count();
            $trans_name = mysqli_real_escape_string($con,$trans_name);
            if($rowcount){
                $a = "UPDATE `".$config['db']['pre']."custom_fields` set 
                translation_lang = '$trans_lang',
                translation_name = '$trans_name' 
                where custom_id = '".$id."' LIMIT 1";
                mysqli_query($con,$a);
            }
            echo 1;
            die();
...
function save_custom_fields()
{
    ...
    foreach($fields as $custom) {
        $id = $custom['id'];
        $type = $custom['type'];
        $title = validate_input($custom['label']);;
        $required = empty($custom['required'])? 0 : $custom['required'];
        $allcat = $custom['allcat'];
        $maincat = $custom['maincat'];
        $category = $custom['services'];
        ...
        if ($type == 'text-field' or $type == 'textarea') {
            ...
            foreach ($custom['items'] as $items) {
                ...
                $opt_id = $items['id'];
                $opt_title = validate_input($items['value']);
                $num_rows = ORM::for_table($config['db']['pre'].'custom_options')
                    ->where('option_id', $opt_id)
                    ->count();
                if($num_rows > 0){

                    $options = ORM::for_table($config['db']['pre'].'custom_options')
                        ->useIdColumn('option_id')
                        ->find_one($opt_id);
                    $options->title = validate_input($opt_title);
                    $options->save();

                }else{
                    $options = ORM::for_table($config['db']['pre'].'custom_options')->create();
                    $options->title = validate_input($opt_title);
                    $options->save();
                    $opt_id = $options->id();
                }
        ...
        if(check_allow()){
            $exist = get_customField_exist_id($id);
            if($exist > 0){
                $fields = ORM::for_table($config['db']['pre'].'custom_fields')
                    ->useIdColumn('custom_id')
                    ->find_one($id);
                $fields->custom_anycat = validate_input($allcat);
                $fields->custom_catid = validate_input($maincat);
                $fields->custom_subcatid = validate_input($category);
                $fields->custom_title = validate_input($title);
                $fields->custom_type = validate_input($type);
                $fields->custom_required = validate_input($required);
                $fields->custom_options = validate_input($options);
                $fields->save();
            }else{
                $lang_code = array();
                $lang_title = array();
                ...
                foreach($info as $fetch){
                    ...
                    $trans_title = $trans->translate($source, $target, $title);*/
                    $trans_title = $title;
                    $trans_title = validate_input($trans_title);
                    $lang_title[] = $trans_title;
                ...
                $fields = ORM::for_table($config['db']['pre'].'custom_fields')->create();
                $fields->translation_lang = validate_input($trans_lang);
                $fields->translation_name = $trans_name;
                $fields->custom_anycat = validate_input($allcat);
                $fields->custom_catid = validate_input($maincat);
                $fields->custom_subcatid = validate_input($category);
                $fields->custom_title = validate_input($title);
                ...
                $fields = ORM::for_table($config['db']['pre'].'custom_fields')
                    ->useIdColumn('custom_id')
                    ->find_one($id);
                $fields->custom_order = validate_input($id);
                $fields->save();
            }
        }
        $count++;
...
function CustomField_Icon_Url()
{
    global $con,$config;

    $id = escape_html($_POST['id']);
    $field_tpl = '<input type="hidden" id="field_id" name="cf_id" value="'.$id.'">';
    if ($id) {
        $sql = "SELECT icon FROM `".$config['db']['pre']."custom_fields` where custom_id = '$id' LIMIT 1";
        $query = mysqli_query($con,$sql);
        $info = mysqli_fetch_assoc($query);

        $field_tpl .= '
        <div class="form-group">
            <label class="col-md-3 control-label">Icon Url</label>
            <div class="col-md-7">
                <input type="text" name="cf_icon" value="'.$info['icon'].'" class="form-control icon_url" placeholder="Icon Image Url">
            </div>
        </div>';
...
function CustomField_Icon_Save()
{
    global $con,$config;

    if (isset($_POST['cf_id'])) {
        $a = "UPDATE `".$config['db']['pre']."custom_fields` set icon = '".escape_html($_POST['cf_icon'])."' where custom_id = '".escape_html($_POST['cf_id'])."' LIMIT 1";
        mysqli_query($con,$a);
        echo 1;
    } else {
        echo 0;
