...
CTR: '.number_format($row["clicks"]*100/$row["shows_displayed"], 2, ".", "").'%';
            ...
            if (check_options() === true) {
                ...
                //$ipaddress = '165.225.106.60';
                require_once  '../../includes/database/geoip/autoload.php';
                // Country DB
                global $config;
                $reader = new \MaxMind\Db\Reader('../../includes/database/geoip/geo_city.mmdb');
                $data = $reader->get($ipaddress);
                $city_id = trim($data['city']['geoname_id']);

                $sql = "SELECT `country_code`, `subadmin1_code` FROM `".$config['db']['pre']."cities` WHERE `id` = '$city_id' LIMIT 1";
                        ...
                        if (empty($banner_details))
                        {
                            ...
                            /*if ($banner_details['country'] == null){
                                ...
                                if ($banner_details['state'] == null){
                                    ...
                                    if ($banner_details['city'] == null){
                                        $res = $banner_details;
                                    }else if ($banner_details['city'] == $city_id){
                                        $res = $banner_details;
                                    }else{
                                        continue;
                                    }
